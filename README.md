...
----
- 1.AES加密内容效率远高于RSA,并且拥有一定的安全性，也使用RAS直接加密data，但数据量大的情况下效率直线下降。
- 2.rsa公钥加密了key，劫持者无法获取key和相关授权信息，保障data的安全
- 3.有授权auth的存在，保障了劫持者无法伪造内容

服务器端  
RSA私钥解密的到AES的KEY和授权Auth和appId ，可以验证本次请求是否有效

通过解密的到AES_key解密加密的data,获取到数据集-->处理结果-->使用AES_key的key加密响应结果，并使用md5hex(data)签名内容，
```
{
